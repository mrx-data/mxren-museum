...
- `src/styles.css` owns the Academia/Classical design system: dark mahogany, aged oak, parchment text, polished brass interactions, paper texture, vignette, ornate dividers, clean rectangular cover framing, and responsive layout. Uploaded covers must not receive category symbols, wax seals, pseudo-element light overlays, ornamental crops, or sepia filters.
...
- `blackMyth.png` is the current bundled final asset. When managed images are added, preserve alt text and render the source image without decorative overlays.
