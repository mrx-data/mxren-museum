...
- Current mode: static frontend application with full-site entry gate, Supabase Postgres metadata, and Supabase Storage images
...
- `src/artifact-store.ts` owns visibility-aware Supabase list/single-artifact queries, tag/date normalization, catalog sorting, custom admin login/session helpers, soft-delete/restore/purge RPC calls, and the browser-local fallback under `mxren-museum.local-artifacts.v1`.
- `src/exhibition-store.ts` owns public/admin exhibition reads, session-protected save/delete RPC calls, ordered artifact references, and the local read fallback under `mxren-museum.local-exhibitions.v1`.
- `src/museum-export.ts` creates the versioned metadata-only JSON archive. It must never include credentials, custom session tokens, Base64 payloads, or image binaries.
...
- `src/main.ts` renders the entry gate, hero poster stage, featured artifacts, category/tag/year facets, catalog sorting, collection cards, exhibition routes, trash/export controls, managed artifacts from Supabase or local fallback, locked/guest/admin access state, management form behavior, visibility controls, `#artifact/{id}` deep links, sharing, and the detail dialog gallery. It calls the motion module after initial render, filter refresh, and dialog operations.
...
- `supabase/migrations/20260715010000_artifact_visibility_and_sharing.sql` adds `draft/published/unlisted`, revokes anonymous table reads, and exposes visibility-aware list, deep-link, and hidden sample-override RPCs.
- `supabase/migrations/20260716010000_catalog_trash_and_exhibitions.sql` adds artifact tags/exact dates, soft-delete recovery RPCs, and the independently managed `museum_exhibitions` table with public/admin read boundaries.
...
- Production Supabase status (2026-07-15): migrations through `20260716010000` are applied and pass remote schema lint; `artifact-images` Function version 1 is active.
- To allow writes, create an admin row in `public.museum_admin_accounts` from Supabase SQL Editor after setting `search_path = public, extensions`, then use `crypt('<admin-password>', gen_salt('bf', 12))`. Do not commit filled password SQL, `sb_secret_...`, or legacy `service_role` keys. Artifact reads and writes both go through their scoped RPC boundaries.
