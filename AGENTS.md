...
- `src/artifact-store.ts` owns visibility-aware Supabase list/single-artifact queries, custom admin login/session helpers, artifact CRUD RPC calls, query helpers, and the browser-local fallback under `mxren-museum.local-artifacts.v1`.
...
- Production Supabase status (2026-07-15): migrations through `20260715010000` are applied and pass remote schema lint; `artifact-images` Function version 1 is active.
